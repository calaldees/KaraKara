...
def _rank(queue: Queue, queue_item: QueueItem) -> Number:
    ...
    def _minuets_ago(d: datetime.datetime):
        return _minuets(queue._now - d) if d else None
    ...
    def _sang_ago_score(i: QueueItem):
        sang_hours_ago = _minuets_ago(i.start_time)  # the start_time could be hypothetical (e.g: it has not been sung, but is scheduled/estimated to be start_time)
        if sang_hours_ago:
...
def reorder(queue: Queue):
    reorder_index = queue.items.index(queue.current) + queue.settings["coming_soon_track_count"]
