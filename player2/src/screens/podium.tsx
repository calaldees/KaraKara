...
export const PodiumScreen = ({state}: { state: State }) => (
    <section key="podium" className={"screen_podium" + (state.queue[0].track.srt_lyrics ? "" : " no_lyrics")}>
        <h1>
            {title_case(get_tag(state.queue[0].track.tags.title))}
            <br/>Performed by <strong>{state.queue[0].performer_name}</strong>
        </h1>
        ...
        {state.playing ?
            ...
            <div className={"startButton"} onclick={(state) => [state, SendCommand(state, "play")]}
