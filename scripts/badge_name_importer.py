...
def _minami_processor(soup):
    ...
    return [
        regex.match(name_string).groupdict()["badge_name"]
        for name_string in (cell.string for cell in soup.find_all("td"))
        if regex.match(name_string)
    ]
...
def _ame_processor(soup):
    regex = DEFAULT_BADGENAME_REGEX
    names = (
        cell.string for cell in soup.find(id="registration-section").find_all("td")
    )
    return [
        regex.match(name_string).groupdict()["badge_name"]
...
processors = {"ame": ame_processor, "minami": minami_processor}
...
def get_args():
    ...
    parser.add_argument(
        "-s", "--source", action="store", help="", choices=processors.keys()
    )
    parser.add_argument(
        "-c",
        "--cache_filename",
